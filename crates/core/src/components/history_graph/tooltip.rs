...
use gpui::{
    Bounds, Pixels, Rgba, SharedString, TextAlign, TextRun, TransformationMatrix, fill, point, px,
    size,
};
...
pub(crate) fn draw_history_graph_tooltip(
    ...
    if bounds.contains(&cursor) && h >= px(60.0) {
        ...
        let hover_color = history_graph_color(value / max_value * 100.0, theme, palette);
        ...
        let secondary_hover = secondary_history
            .as_ref()
            .map(|(secondary_history, color)| {
                ...
                let secondary_cursor_step =
                    (cursor.x - left + secondary_step * sub_ratio) / secondary_step;
                let secondary_index = stepped_history_index(secondary_cursor_step, secondary_count);
                (
                    secondary_history[secondary_index].clamp(0.0, max_value),
                    *color,
                )
            });
        let tooltip_count = if secondary_hover.is_some() { 2.0 } else { 1.0 };
        let tooltip_stack_height =
            tooltip_size.height * tooltip_count + gap * (tooltip_count - 1.0);
        let tooltip_stack_y = (cursor.y - tooltip_stack_height / 2.0)
            .max(top + gap)
            .min(top + h - gap - tooltip_stack_height);

        for (tooltip_index, (value, color)) in std::iter::once((value, hover_color))
            ...
            .enumerate()
        {
            ...
            window.paint_quad(
                fill(
                    Bounds {
                        origin: point(cursor.x - px(4.0), point_y - px(4.0)),
            ...
            let tooltip_y = tooltip_stack_y + (tooltip_size.height + gap) * tooltip_index as f32;
            let tooltip_bounds = Bounds {
                origin: point(tooltip_x, tooltip_y),
                size: tooltip_size,
            };
            window.paint_quad(fill(tooltip_bounds, color.opacity(0.75)).corner_radii(px(6.0)));
            window.paint_quad(
                fill(
                    Bounds {
                        origin: tooltip_bounds.origin + point(px(1.0), px(1.0)),
            ...
            let line = window
                .text_system()
                .shape_line(label, px(11.0), &[text_run], None);
            ...
            let text_x = if let Some(icon_path) = icon_path {
                ...
                let icon_x = tooltip_x + (tooltip_size.width - content_width) / 2.0;
                window
                    .paint_svg(
                        Bounds {
                            origin: point(
                                icon_x,
                                tooltip_y + (tooltip_size.height - icon_size) / 2.0,
                            ),
