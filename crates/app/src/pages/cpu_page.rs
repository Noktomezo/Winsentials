...
use gpui::{
    Animation, AnimationExt, App, Background, Bounds, ContentMask, FontWeight, Interpolate,
    IntoElement, ParentElement, PathBuilder, Pixels, RenderOnce, Rgba, SharedString, Styled,
    TextAlign, TextRun, TransformationMatrix, Window, canvas, div, fill, linear_color_stop,
...
fn stepped_history_index(cursor_step: f32, count: usize) -> usize {
    (0..count)
        .position(|index| cursor_step <= (index + 1) as f32)
        .unwrap_or_else(|| count.saturating_sub(1))
}

fn stepped_corner_radius(step_width: Pixels, y_curr: Pixels, y_next: Pixels) -> gpui::Size<Pixels> {
    let y_min = y_curr.min(y_next);
    let height = y_curr.max(y_next) - y_min;
    size(px(4.0).min(step_width / 2.0), px(4.0).min(height / 2.0))
}

fn smooth_percent_transition(from: f32, to: f32, progress: f32) -> f32 {
...
const GRAPH_COLOR_BANDS: [(f32, f32); 5] = [
    (0.0, 55.0),
    (55.0, 65.0),
    (65.0, 80.0),
    (80.0, 90.0),
...
const SOLID_COLOR_BANDS: [(f32, f32); 1] = [(0.0, 100.0)];

#[derive(Clone, Copy)]
pub(crate) enum HistoryGraphPalette {
    Semantic,
    Solid(Rgba),
}

fn graph_percent_color(percent: f32, theme: &Theme) -> Rgba {
    if percent < 55.0 {
        theme.accent_green
...
fn history_graph_color(percent: f32, theme: &Theme, palette: HistoryGraphPalette) -> Rgba {
    match palette {
...
pub(crate) fn render_stepped_history_graph(
    history: &[f32],
    ...
    let history_data: Arc<[f32]> = Arc::from(history);
    let secondary_history: Option<(Arc<[f32]>, Rgba)> =
        secondary_history.map(|(history, color)| (Arc::from(history), color));
    let theme = *theme;
                ...
                graph.child(
                    canvas(
                        ...
                        move |bounds, (), window, cx| {
                            ...
                            // 1. Subtle background grid lines (25%, 50%, 75%)
                            ...
                            // 2. Continuous time-smooth conveyor glide anchored to the exact sample instant
                            ...
                            let line_path = line_path
                                .build()
                                .expect("CPU graph path geometry must be valid");
                            let path_bottom =
                            ...
                            let halo_size = px(8.0 + 8.0 * pulse);
                            window.paint_quad(
                                fill(
                                    Bounds {
                                        origin: point(
                            ...
                            window.paint_quad(
                                fill(
                                    Bounds {
                                        origin: point(live_x - px(4.0), live_y - px(4.0)),
